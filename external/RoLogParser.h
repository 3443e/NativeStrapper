...
static inline RoLogObject* RoLogCreateObject(char* file_path) {
    RoLogObject* obj = (RoLogObject*)malloc(sizeof(RoLogObject));
    if (!obj) return NULL;

    memset(obj, 0, sizeof(RoLogObject));

    if (file_path) {
        obj->file_path = strdup(file_path);
    } else {
        obj->file_path = NULL;
    }
    ...
    memset(obj->serverObj, 0, sizeof(RoLogServerIPs));

    obj->serverObj->serverIPRCC = (char*)calloc(64, 1);
    obj->serverObj->serverIPUDMUX = (char*)calloc(64, 1);

    obj->serverObj->serverType = ROLOGINVALIDSERVER;
...
static inline void RoLogParseLine(RoLogObject* obj, const char* line) {
    if (!obj || !line) return;

    char buffer[MAX_LINE_BUFFER_LENGHT];
    strncpy(buffer, line, sizeof(buffer) - 1);
    buffer[sizeof(buffer) - 1] = '\0';

    if (strstr(buffer, "returnToLuaApp")) {
        obj->current_state = ROLOG_IN_LUA_APP;
    }

    if (strstr(buffer, "setStage: (stage:None)")) {
        obj->current_state = ROLOG_OFFLINE;
    }

    if (strstr(buffer, "Report game_join_loadtime")) {
        obj->current_state = ROLOG_IN_GAME;
        RoLogDealWithGameJoinLoadTimeValues(obj, buffer);
    }

    if (strstr(buffer, "Joining game '")) {
        RoLogDealWithJoiningGame(obj, buffer);
    }

    if (strstr(buffer, "Connecting to")) {
        RoLogDealWithConnectingTo(obj, buffer);
    }

    if (strstr(buffer, "UDMUX Address")) {
        RoLogDealWithUDMUXAddress(obj, buffer);
    }

    if (strstr(buffer, "Server RobloxGitHash")) {
        free(obj->serverRobloxGitHash);
        obj->serverRobloxGitHash = RoLogExtractAfterColon(buffer, "Server RobloxGitHash");
    }

    if (strstr(buffer, "Server Prefix")) {
        free(obj->serverPrefix);
        obj->serverPrefix = RoLogExtractAfterColon(buffer, "Server Prefix");

        free(obj->serverLuauVersion);
        obj->serverLuauVersion = RoLogExtractLuauVersionFromGitHash(obj->serverPrefix);
    }

    if (strstr(buffer, "[FLog::ClientRunInfo] RobloxGitHash")) {
        free(obj->clientRobloxGitHash);
        obj->clientRobloxGitHash = RoLogExtractAfterColon(buffer, "RobloxGitHash");
    }

    if (strstr(buffer, "[FLog::ClientRunInfo] The channel is")) {
        RoLogDealWithChannel(obj, buffer);
    }
    if (strstr(buffer, "resizing main targets to")) {
        RoLogDealWithResolution(obj, buffer);
    }
}
